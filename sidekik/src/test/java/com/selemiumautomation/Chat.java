...
public class Chat {
    public static void chatHandler(WebDriver driver, PrintWriter logWriter) {
        ...
        try {
            ...
            WebElement chatButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(text(), 'Chat')]")));
            chatButton.click();
            logWriter.println("💬 Clicked on the chat button.");

            // Wait for chat list to be visible
            @SuppressWarnings("unused")
            WebElement chatList = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class,'overflow-auto custom-scrollbar')]")));
            logWriter.println("✅ Chat list is visible.");

            // Select the first chat from the list
            List<WebElement> chatItems = driver.findElements(By.xpath("//div[contains(@class,'overflow-auto custom-scrollbar')]/div"));
            ...
            @SuppressWarnings("unused")
            WebElement chatWindow = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class,'flex flex-col h-full w-4/5')]")));
            logWriter.println("✅ Chat window is open.");
            Thread.sleep(1000);

            // Locate message input box
            WebElement messageInput = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@placeholder='Type a message...']")));
            ...
            WebElement sendButton = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//button[contains(@class, 'inline-flex') and contains(@class, 'justify-center') and contains(@class, 'p-2')]")));
            sendButton.click();
            logWriter.println("📩 Clicked send button.");

            // Verify the message appears in the chat
            WebElement sentMessage = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[contains(text(),'Hello, this is a test message!')]")));
